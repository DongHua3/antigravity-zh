...
class TranslationEngine {
  ...
  setupRootObserver() {
    ...
    if (this.window && typeof this.window.addEventListener === 'function') {
      ...
      const delays = [100, 300, 1000, 3000, 6000];
  ...
  lookupTranslation(text) {
    ...
    // 5. Substring sliding match and 18-character prefix match for long paragraphs
    if (this.longEntries && this.longEntries.length > 0) {
      for (const [key, replacement] of this.longEntries) {
        ...
        } else if (key.length >= 18 && normalized.length >= 18 && lowerNorm.slice(0, 18) === key.slice(0, 18).toLowerCase()) {
          return formatResult(replacement);
        }
      }
    }
