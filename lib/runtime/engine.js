...
class TranslationEngine {
    ...
    for (const dictKey of dictOrder) {
      ...
      if (d) {
        for (const [k, v] of Object.entries(d)) {
          ...
          if (k.length > 15 && !seenLongKeys.has(k)) {
            seenLongKeys.add(k);
            this.longEntries.push([k, v]);
          }
        }
      }
    }
  ...
  lookupTranslation(text) {
    ...
    if (trimmed.toLowerCase() === 'antigravity') {
      if (this.brandTitle === 'hidden') return text.replace(trimmed, '');
      if (this.brandTitle === 'english') return text;
      if (this.brandTitle === 'translated') return text.replace(trimmed, '反重力智能编程');
    }
    ...
    if (this.lowerMap && this.lowerMap.has(lowerTrimmed)) {
      return text.replace(trimmed, this.lowerMap.get(lowerTrimmed));
    }

    // 3. Keyboard shortcut suffix matching: e.g. "Command Palette (Ctrl+Shift+P)" or "New Tab (⌘T)"
    const shortcutMatch = trimmed.match(/^(.+?)\s*\(((?:Ctrl|Cmd|Alt|Shift|⌘|⌥|⇧|⌃)(?:[+A-Za-z0-9_-]+)?)\)$/i);
    if (shortcutMatch) {
      const prefix = shortcutMatch[1].trim();
      const shortcutPart = shortcutMatch[2];
      const transPrefix = this.lookupTranslation(prefix);
      if (transPrefix) {
        return text.replace(trimmed, `${transPrefix.trim()} (${shortcutPart})`);
      }
    }
    ...
    if (this.longEntries && trimmed.length > 15) {
