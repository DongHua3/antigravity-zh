...
const { patchUpdater } = require('./patches/updater');
 ...
 * Cleanly strips any previous injection so re-patching always installs the latest version.
 * Appends to the bottom of dist/preload.js so all contextBridge APIs are established first.
 ...
 */
function patchPreload(preloadSource, dictionaries, options = {}) {
  if (typeof preloadSource !== 'string') {
    throw new TypeError('preloadSource must be a string');
  }

  // 1. Clean any previous injection blocks
  let cleaned = preloadSource;
  const sigStart = '/* --- ANTIGRAVITY CHINESE LOCALIZATION START --- */';
  const sigEnd = '/* --- ANTIGRAVITY CHINESE LOCALIZATION END --- */';
  const sigStartIdx = cleaned.indexOf(sigStart);
  const sigEndIdx = cleaned.indexOf(sigEnd);
  if (sigStartIdx !== -1 && sigEndIdx !== -1 && sigStartIdx < sigEndIdx) {
    cleaned = cleaned.substring(0, sigStartIdx) + cleaned.substring(sigEndIdx + sigEnd.length);
  }
  // Also clean old un-delimited IIFE if present
  if (cleaned.includes('__initAntigravityZhEngine__')) {
    cleaned = cleaned.replace(/\/\*\*[\s\S]*?__initAntigravityZhEngine__[\s\S]*?\}\)\(\);\s*/g, '');
  }

  // 2. Generate updated injection bundle and append to the bottom
  const injectionBundle = getInjectedPreloadSource(dictionaries, options);
  ...
  const patchRegistry = [
    {
      ...
      fn: (content) => patchPreload(content, dicts, options)
    },
    {
    ...
    {
      relPath: path.join('dist', 'updater.js'),
      name: 'Updater Dialogs & Menu Labels',
      fn: patchUpdater
    }
  ];
